#!/usr/bin/env python3
"""R6.94: Rebuild gw-frontend image + recreate + verify smoketest (USER-AUTHORIZED)."""
import re, time
...
# Pre-check: host entrypoint has R6.93 fix
print('[pre-check] host entrypoint R6.93 fix:')
stdin, stdout, stderr = tg.exec_command(
    'grep -n "TEMPLATES_DIR" /home/zjlab/gravitationalwave-v4.31/gw-frontend/docker-entrypoint.sh 2>&1')
...
print('=== BUILD OUTPUT (last 2500 chars) ===')
print(out_safe[-2500:] if len(out_safe) > 2500 else out_safe)
ch2.close()
...
print()
print('[verify] smoketest via 6001 (follow redirect):')
stdin, stdout, stderr = tg.exec_command(
