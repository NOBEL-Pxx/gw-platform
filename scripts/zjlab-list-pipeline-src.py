...
cmds = [
    'echo === pipeline source dir tree (host) ===',
    'find /home/zjlab/gravitationalwave-v4.31/gw-pipeline -maxdepth 4 -type f 2>&1 | head -30',
    'echo === middleware specifically ===',
    'find /home/zjlab/gravitationalwave-v4.31/gw-pipeline -path "*middleware*" 2>&1',
    'echo === image id (sha) of currently-running pipeline ===',
    'docker inspect gw-pipeline --format="{{.Image}}"',
    'echo === image build history ===',
    'docker image history gravitationalwave-v431-gw-pipeline 2>&1 | head -10',
]
for c in cmds:
    print('--- ' + c + ' ---')
