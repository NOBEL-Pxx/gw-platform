"""R6.94 final: clean stale template (via container, since bind-mounted) + recreate + verify."""
import re, time
...
print('[1] delete stale r692-smoketest (no .conf) from bind mount via container:')
stdin, stdout, stderr = tg.exec_command(
    'docker exec gw-frontend sh -c "rm -f /etc/nginx/conf.d/r692-smoketest && rm -f /etc/nginx/conf.d/templates/r692-smoketest.template && ls /etc/nginx/conf.d/templates/"')
print(stdout.read().decode(errors='replace').rstrip())

print()
print('[2] recreate gw-frontend (entrypoint regens confs):')
cmd = ('cd ' + REMOTE_ROOT + ' && '
...
print('[3] entrypoint logs (envsubst trace):')
stdin, stdout, stderr = tg.exec_command(
    'docker logs gw-frontend 2>&1 | grep -i "envsubst" | head -10')
print(stdout.read().decode(errors='replace').rstrip())

print()
print('[4] r692-smoketest.conf contents:')
stdin, stdout, stderr = tg.exec_command(
...
print()
print('[6] smoketest via HTTP 6001 (no -L):')
stdin, stdout, stderr = tg.exec_command(
    'curl -sk -w "\nHTTP=%{http_code}\n" http://localhost:6001/r692-smoketest-status 2>&1 | tail -3', timeout=15)
print(stdout.read().decode(errors='replace').rstrip())

print()
print('[7] smoketest via HTTPS 6002:')
stdin, stdout, stderr = tg.exec_command(
    'curl -sk --resolve "localhost:6002:127.0.0.1" -w "\nHTTP=%{http_code}\n" https://localhost:6002/r692-smoketest-status 2>&1 | tail -3', timeout=15)
print(stdout.read().decode(errors='replace').rstrip())
