...
//   2. Hardcoded fallback `'v4.63+R6.85'` - only used in dev when git tags
//      don't exist AND pkg.version is missing AND no env var.
//
// Why a shared module instead of inline `import.meta.env` in sentry.ts:
//   - Before R6.71: useFontMonitor.ts had `const APP_VERSION = 'v4.63+R6.85'`
...
export const APP_VERSION: string =
  import.meta.env.VITE_APP_VERSION || 'v4.63+R6.85'
